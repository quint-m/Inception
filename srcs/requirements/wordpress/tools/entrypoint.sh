...
if ! wp core is-installed 2>/dev/null; then
    ...
    wp core install --url=localhost --title="Inception" --admin_user=${WP_ADMIN} --admin_password=$(cat /run/secrets/wp_admin_password) --admin_email=qmennen@student.codam.nl --skip-email --allow-root
